...
Moved from Dev-Centr (`dev-centr/prohelp`) into OpenShellOrg as the reusable progressive-help library and global `help` entry point.
...
* **D (recommended):** link:https://github.com/openshellorg/prohelp-cli[prohelp-cli] with link:https://opendlang.org/library/arsd.cli.html[arsd.cli] (or another D CLI router) — parsing and routing stay in the framework; help stays in Prohelp.
...
For new D command-line tools, prefer link:https://github.com/openshellorg/prohelp-cli[prohelp-cli] integrated with a CLI router such as link:https://opendlang.org/library/arsd.cli.html[arsd.cli]. See the prohelp-cli repository for how execution routing and progressive help cooperate.
...
Resolution order: adjacent `help.sdl` / orientation pack → OpenShellOrg `info` TUI or classic `info`/`man` → `<cmd> --help`.
